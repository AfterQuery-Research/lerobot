...
from lerobot.robots.bi_yam.config_bi_yam import BI_YAM_MAX_GRIPPER_POSITION, YAM_SCALAR_KEYS
from lerobot.robots.bi_yam.kinematics import I2RT_YAM_JOINT_LIMITS, I2RTYAMKinematics

from .umi_yam import ACTION_HORIZON, decode_bimanual_action, encode_bimanual_action
...
class BiYAMActionAdapter:
    """Adapt query-anchored UMI EE rows to safe BiYAM joint commands."""
    ...
    def prepare_observation(self, current_state: object) -> PreparedBiYAMObservation:
        ...
        model_state = encode_bimanual_action(
            ...
            left_gripper=np.asarray(min(current[6] / BI_YAM_MAX_GRIPPER_POSITION, 1.0)),
            right_delta_transform=identity,
            right_gripper=np.asarray(min(current[13] / BI_YAM_MAX_GRIPPER_POSITION, 1.0)),
        )
        anchor = BiYAMQueryAnchor(
    ...
    def to_joint_action(self, action: object, anchor: BiYAMQueryAnchor | None) -> np.ndarray:
        ...
        command = np.concatenate(
            (
                left_q,
                [left_gripper * BI_YAM_MAX_GRIPPER_POSITION],
                right_q,
                [right_gripper * BI_YAM_MAX_GRIPPER_POSITION],
            )
        )
