...
class RemoteInferenceEngine(InferenceEngine):
    ...
    def __init__(
        ...
        *,
        ...
        if action_adapter is not None and settings.execution_horizon != action_adapter.action_horizon:
            raise ValueError(
                "remote action adapter requires execution_horizon="
                f"{action_adapter.action_horizon}, got {settings.execution_horizon}"
            )
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if (
                self._action_adapter is not None
                and session.model.action_horizon != self._action_adapter.action_horizon
            ):
                raise ValueError(
                    "remote BiYAM EE model must have action_horizon="
                    f"{self._action_adapter.action_horizon}, got {session.model.action_horizon}"
                )
