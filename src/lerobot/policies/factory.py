...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if pretrained_path:
        ...
        elif policy_cfg.type == "molmoact2":
            tokenizer_overrides = preprocessor_overrides.setdefault("molmoact2_pack_inputs", {})
            for field_name in (
                "discrete_action_tokenizer",
                "discrete_action_tokenizer_revision",
                "discrete_action_tokenizer_load_path",
            ):
                value = getattr(policy_cfg, field_name, None)
                if value is not None:
                    tokenizer_overrides.setdefault(field_name, value)
