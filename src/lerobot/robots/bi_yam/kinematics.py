...
class I2RTYAMKinematics:
    ...
    ``ik`` requires an explicit seed so sequential dataset materialization can
    carry each arm's previous solution instead of silently resetting branches.
    It raises on solver failure or any invalid result rather than returning an
    ...
    def ik(self, target_pose: object, seed_q: object) -> np.ndarray:
        ...
        success, raw_solution = self._backend.ik(
            ...
            pos_threshold=self.position_tolerance_m,
            ori_threshold=self.orientation_tolerance_rad,
        )
        if not success:
            raise RuntimeError("I2RT IK failed to converge")
