...
Select a backend with `--inference.type=<name>`. All strategies work with each backend.
...
### Remote (`--inference.type=remote`)

Remote inference keeps the policy and GPU on a policy server while `lerobot-rollout`
runs on the robot computer. The client sends typed observations and asynchronously
queues returned action chunks. Do not pass `--policy.path` to the client; select the
checkpoint when starting `lerobot-policy-server`.
...
See [Remote Policy Inference](./remote_inference) for server setup, dual-YAM
hardware configuration, and simulator testing.
...
| --------------------------------- | ----------------------------------------------------------------- | ------- |
| `--policy.path`                   | Required for local inference; omitted for remote inference        | --      |
