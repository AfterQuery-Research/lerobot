...
For a GPU policy server on a different machine, including the original
`allenai/MolmoAct2` checkpoint and the dual-YAM adapter, see
[Remote Policy Inference](./remote_inference).
