...
16 to 32 is a good starting point. In these settings,
`policy.train_mode_vlm=lora` or `policy.train_mode_vlm=freeze` is also a
practical choice. LoRA on the VLM keeps the action expert fully trainable,
which we found to be crucial for model performance. For larger fine-tuning
datasets, larger global batch sizes and full fine-tuning are usually preferred.
...
- `policy.action_mode`: training target, one of `continuous`, `discrete`, or
  ...
  action-token loss.
- `policy.train_mode_vlm`: VLM training mode, one of `fft`, `lora`, or
  `freeze`. `fft` is the backward-compatible default. `lora` adapter-tunes the
  VLM while fully fine-tuning the action expert. `freeze` trains only the action
  expert and requires `policy.action_mode=continuous`.
- `policy.enable_lora_vlm`, `policy.enable_lora_action_expert`, and
  `policy.train_action_expert_only`: deprecated checkpoint/config aliases kept
  for backward compatibility. New recipes should use `policy.train_mode_vlm`.
  Legacy action-expert LoRA remains loadable when VLM LoRA is enabled, but is
  not recommended for new fine-tuning runs.
...
- `policy.llm_residual_dropout`: residual dropout applied to the text
  transformer while training. The backward-compatible default is `0.0`;
  set it explicitly when a recipe requires additional regularization.
...
- LoRA VLM fine-tuning sets the VLM, vision, and connector LoRA parameter
  groups to `5e-5` when `policy.train_mode_vlm=lora`. The action expert is
  still fully fine-tuned with `policy.optimizer_action_expert_lr` unless an old
  checkpoint explicitly enables the deprecated action-expert LoRA mode.
